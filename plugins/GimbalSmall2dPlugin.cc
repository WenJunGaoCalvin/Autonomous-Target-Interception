...
void GimbalSmall2dPlugin::Init()
{
  ...
  this->dataPtr->ros_sub = this->dataPtr->nh.subscribe("/ros_pitch", 1, 
    &GimbalSmall2dPluginPrivate::Activate_Callback, this->dataPtr.get());

  this->dataPtr->ros_pub =
    this->dataPtr->nh.advertise<std_msgs::String>("/ros_gimbal_pitch_status", 1);
}
...
/////////////////////////////////////////////////
void GimbalSmall2dPluginPrivate::Activate_Callback(const std_msgs::Float64::ConstPtr& msg)
{
  // ROS_INFO("Received Message");
  // ROS_INFO("Received Message = %g", msg->data);
  this->pitch_ros = msg->data;
}
...
void GimbalSmall2dPlugin::OnUpdate()
{
  ...
  else if (time > this->dataPtr->lastUpdateTime)
  {
    ...
    double pitch_setpt = this->dataPtr->pitch_ros;
    double error = angle - pitch_setpt;
  ...
  if (++i > 100)
  {
    ...
    this->dataPtr->ros_current_pitch.data = to_string(angle);
    this->dataPtr->ros_pub.publish(this->dataPtr->ros_current_pitch);
  }
  IGN_PROFILE_END();
}
