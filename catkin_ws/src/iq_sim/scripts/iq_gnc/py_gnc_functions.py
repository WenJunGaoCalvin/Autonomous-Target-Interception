...
class gnc_api:
    def __init__(self,ns):
        ...
        self.gimbal_yaw_rate_pub = rospy.Publisher(
            name='/ros_yaw_rate',
            data_class=Float64,
            queue_size = 10
        )

        self.gimbal_pitch_pub = rospy.Publisher(
            name='/ros_pitch',
            data_class=Float64,
            queue_size = 10
        )

        self.currentGimbalYaw = rospy.Subscriber(
            name="/ros_gimbal_yaw_status",
            data_class= String,
            queue_size=10,
            callback=self.gimbal_cb,
        )
    ...
    def wait4start(self):
        ...
        while not rospy.is_shutdown() and self.current_state_g.mode != "GUIDED":
            self.set_mode_client.call(mode)
            rospy.sleep(0.01)
        else:
